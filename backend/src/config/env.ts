...
const envSchema = z.object({
   ...
   * `exp://<lan-ip>:8081/--/auth/callback`. That is refused in production on
   * purpose — it carries an arbitrary host, so accepting it unconditionally
   * would be an open redirect.
   *
   * Turn on only while testing on a real phone, and off again afterwards.
   *
   * NOT z.coerce.boolean(): coercion follows JavaScript truthiness, so the
   * string "false" — the natural way to switch this off in a .env file —
   * becomes `true`. A security flag that cannot be turned off is worse than
   * no flag. Match the literal instead.
   */
